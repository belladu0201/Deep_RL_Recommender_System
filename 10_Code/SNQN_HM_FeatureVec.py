    ...
    def __init__(
        ...
        with tf.variable_scope(self.name):
            ...
            self.output2 = tf.compat.v1.layers.dense(
                self.states_hidden, self.item_num, use_bias=True, activation=None
            )  # all ce logits - should be phi score. (batch_size, item_num)
            ...
            self.w_f = tf.compat.v1.layers.dense(
                ...
                use_bias=True,
                activation=None,
            )  # (item_num, hidden size)

            self.phi_2 = tf.matmul(
                self.states_hidden, self.w_f, transpose_b=True
            )  # (batch_size, item_num) - same shape as self.output2

            self.final_score = (
                1 - self.lambda_score
            ) * self.output2 + self.lambda_score * self.phi_2  # (batch_size, item_num)
...
def evaluate(sess):
    ...
    eval_ids = eval_sessions.customer_id.unique()
    groups = eval_sessions.groupby("customer_id")
    batch = 100
    evaluated = 0
    total_clicks = 0.0
    total_purchase = 0.0
...
if __name__ == "__main__":
    ...
    feature_matrix = pd.read_csv(
        os.path.join(data_directory, "feature_matrix_hm.csv")
    ).values.astype('float32')  # one-hot-encoded feature matrix
